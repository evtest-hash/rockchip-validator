...
enum CLI {
    ...
    private static var usage: String { """
        ...
        用法：
          rkval devices [--model AZ08]        列出当前处于 maskrom 的板卡
          rkval run --model AZ08 [选项]       对一块板跑完验证序列，输出报告
          rkval report <run.json>            重新渲染一次已归档的运行
          rkval help                         显示本说明
            ...
            if let given = o.string("device-id") {
                ...
                guard devices.count == 1 else {
                    return fail(devices.isEmpty
                        ? "当前没有处于 maskrom 的板卡。"
                        : "有 \(devices.count) 块板在位，请用 --device-id 指定；`rkval devices` 可列出。")
                }
