...
/// of substring assertions. Skipped unless REPORT_DUMP is set; it is a tool, not a check.
final class GoldenReportDump: XCTestCase {
    func testDumpASampleRun() throws {
        guard let dest = ProcessInfo.processInfo.environment["REPORT_DUMP"] else {
            throw XCTSkip("set REPORT_DUMP to a path")
        }
        ...
        let run = Run(schemaVersion: Run.currentSchema, runID: "1787190336-FB1391E4",
                      ...
                      burninPhases: BurninPhase.allCases, scale: .default, items: TestItem.ddrItems,
                      results: results,
