...
final class QuitGuardTests: XCTestCase {
    ...
    private static let plan = RunPlan(batchID: "b", model: .az08, flow: .ddr,
                                      items: TestItem.ddrItems,
                                      burninPhases: Set(BurninPhase.allCases),
                                      deviceID: "002-1.4")

    private func bench(startedAgo seconds: TimeInterval?, now: Date) -> Bench {
        let b = Bench(deviceID: Self.plan.deviceID, plan: Self.plan)
    ...
    func testNothingRunningQuitsWithoutAsking() {
        XCTAssertNil(QuitGuard.quitWarning(forQuitting: [], now: Date()))
    }
    ...
    func testAFinishedBoardIsNotSomethingToLose() {
        ...
        b.isFinished = true
        XCTAssertNil(QuitGuard.quitWarning(forQuitting: [b], now: now),
                     "报告已经写完了，退出不损失任何东西")
    }
    ...
    func testOneRunningBoardIsNamedWithWhatItHasSpent() {
        let now = Date()
        let m = QuitGuard.quitWarning(forQuitting: [bench(startedAgo: 3 * 3600 + 12 * 60, now: now)],
                                      now: now)
        XCTAssertNotNil(m)
    ...
    /// One board must not be told 「它们」. The message counts instead of pronouning, and this is
    /// the case that made the difference visible — it only showed up on the rendered alert.
    func testTheWordingHoldsAtOneBoard() {
        let now = Date()
        let m = QuitGuard.quitWarning(forQuitting: [bench(startedAgo: 8, now: now)], now: now) ?? ""
        XCTAssertFalse(m.contains("它们"), "一块板不能说「它们」：\n\(m)")
        XCTAssertTrue(m.contains("这 1 块板需要从头重新验证"), m)
    }
    ...
    func testTheElapsedTimeIsTheEarliestStart() {
        ...
        let m = QuitGuard.quitWarning(forQuitting: benches, now: now)
    ...
    func testARunningBoardWithNoStartTimeStillCounts() {
        let now = Date()
        let m = QuitGuard.quitWarning(forQuitting: [bench(startedAgo: nil, now: now)], now: now)
    ...
    func testOnlyRunningBoardsAreCounted() {
        ...
        let m = QuitGuard.quitWarning(forQuitting: [running, done, running], now: now)
    ...
    /// No state attached means no batch was ever started, so there is nothing to guard. This also
    /// covers a quiet bench: `app?.allBenches ?? []` reaches the same empty array either way.
